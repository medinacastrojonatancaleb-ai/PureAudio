...
export default function LibraryScreen() {
  ...
  return (
    <div className="space-y-12 pb-20">
      ...
      {/* Premium Settings */}
      {isPremium && (
        <section className="bg-gradient-to-br from-[#1d1d1d] to-[#121212] rounded-xl p-6 border border-primary/20 space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
               <div className="bg-primary/20 p-2 rounded-lg text-primary">
                  <Settings size={20} />
               </div>
               <h2 className="text-xl font-black tracking-tight">{t('premium_features')}</h2>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="flex items-center justify-between p-4 bg-white/5 rounded-lg border border-white/5">
               <div>
                  <p className="font-bold text-sm">{t('high_quality')}</p>
                  <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-0.5">320kbps Audio</p>
               </div>
               <button 
                 onClick={toggleHighQuality}
                 className={`w-12 h-6 rounded-full transition-colors relative ${isHighQuality ? 'bg-primary' : 'bg-white/10'}`}
               >
                 <div className={`absolute top-1 bottom-1 w-4 rounded-full transition-all ${isHighQuality ? 'right-1 bg-blackScale-90' : 'left-1 bg-white'} shadow-lg`} />
               </button>
            </div>
            <div className="flex items-center justify-between p-4 bg-white/5 rounded-lg border border-white/5">
               <div>
                  <p className="font-bold text-sm">{t('ad_free')}</p>
                  <p className="text-[10px] text-gray-500 uppercase tracking-widest mt-0.5">{t('active')}</p>
               </div>
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-xl font-black tracking-tight">{t('offline_play')}</h2>
            </div>
            <span className="text-xs font-bold text-gray-500 uppercase">
              {downloadedTracks.length} {t('songs')}
            </span>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {downloadedTracks.map((track) => (
              <motion.div 
                whileTap={{ scale: 0.98 }}
                key={`dl-${track.id}`} 
                className="flex items-center gap-4 p-2 rounded-lg bg-white/5 hover:bg-white/10 transition-all group border border-white/[0.02] cursor-pointer"
              ...
              >
                <div className="relative w-12 h-12 flex-shrink-0">
                  ...
                  <div className="absolute inset-x-0 bottom-0.5 h-1 bg-primary/40 rounded-full mx-1 scale-x-50" />
                </div>
        ...
        {likedTracks.length > 0 ? (
          ...
          <div className="py-20 flex flex-col items-center justify-center gap-6 bg-white/[0.02] rounded-2xl border-2 border-dashed border-white/5">
            ...
            <div className="text-center max-w-xs">
               <p className="text-lg font-bold text-white">{user ? t('songs_appear_here') : t('sync_library')}</p>
               <p className="text-sm text-gray-500 mt-1">
                 {user ? t('tap_heart_help') : t('easy_help')}
               </p>
               {!user && (
                 <button 
                   onClick={login}
                   className="mt-6 flex items-center justify-center gap-3 bg-white text-black font-black px-8 py-3 rounded-full hover:scale-105 active:scale-95 transition-all shadow-xl"
                 >
